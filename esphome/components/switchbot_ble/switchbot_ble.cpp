...
// bool parse_switchbot_value(uint16_t value_type, const uint8_t *data, uint8_t value_length, SwitchbotParseResult
// &result) {
...
//   //   ESP_LOGVV(TAG, "parse_switchbot_message(): payload is encrypted, stop reading message.");
//   //   return false;
//   // }
...
//   if (payload_length < 4) {
//     ESP_LOGVV(TAG, "parse_switchbot_message(): payload has wrong size (%d)!", payload_length);
//     return false;
//   }
...
//       ESP_LOGVV(TAG, "parse_switchbot_message(): fixed byte not found, stop parsing residual data.");
//       break;
//     }

//     const uint8_t value_length = payload[payload_offset + 2];
//     if ((value_length < 1) || (value_length > 4) || (payload_length < (3 + value_length))) {
//       ESP_LOGVV(TAG, "parse_switchbot_message(): value has wrong size (%d)!", value_length);
//       break;
//     }
...
//     if (parse_switchbot_value(value_type, data, value_length, result))
//       success = true;
...
//     ESP_LOGVV(TAG, "parse_switchbot_header(): no service data UUID magic bytes.");
//     return {};
//   }
...
//   if (!result.has_data) {
//     ESP_LOGVV(TAG, "parse_switchbot_header(): service data has no DATA flag.");
//     return {};
//   }

//   static uint8_t last_frame_count = 0;
//   if (last_frame_count == raw[4]) {
//     ESP_LOGVV(TAG, "parse_switchbot_header(): duplicate data packet received (%d).",
//     static_cast<int>(last_frame_count)); result.is_duplicate = true; return {};
//   }
...
//     result.type = SwitchbotParseResult::TYPE_HHCCJCY01;
//     result.name = "HHCCJCY01";
//   } else if (device_uuid == 0x01aa) {  // round body, segment LCD
//     result.type = SwitchbotParseResult::TYPE_LYWSDCGQ;
//     result.name = "LYWSDCGQ";
//   } else if (device_uuid == 0x015d) {  // FlowerPot, RoPot
//     result.type = SwitchbotParseResult::TYPE_HHCCPOT002;
//     result.name = "HHCCPOT002";
//   } else if (device_uuid == 0x02df) {  // Switchbot (Honeywell) formaldehyde sensor, OLED display
//     result.type = SwitchbotParseResult::TYPE_JQJCY01YM;
//     result.name = "JQJCY01YM";
//   } else if (device_uuid == 0x03dd) {  // Philips/Switchbot BLE nightlight
//     result.type = SwitchbotParseResult::TYPE_MUE4094RT;
...
//     result.type = SwitchbotParseResult::TYPE_CGG1;
//     result.name = "CGG1";
//   } else if (device_uuid == 0x03bc) {  // VegTrug Grow Care Garden
//     result.type = SwitchbotParseResult::TYPE_GCLS002;
//     result.name = "GCLS002";
//   } else if (device_uuid == 0x045b) {  // rectangular body, e-ink display
//     result.type = SwitchbotParseResult::TYPE_LYWSD02;
//     result.name = "LYWSD02";
//   } else if (device_uuid == 0x2542) {  // rectangular body, e-ink display — with bindkeys
//     result.type = SwitchbotParseResult::TYPE_LYWSD02MMC;
//     result.name = "LYWSD02MMC";
//     if (raw.size() == 19)
//       result.raw_offset -= 6;
//   } else if (device_uuid == 0x040a) {  // Mosquito Repellent Smart Version
//     result.type = SwitchbotParseResult::TYPE_WX08ZM;
//     result.name = "WX08ZM";
//   } else if (device_uuid == 0x0576) {  // Cleargrass (Qingping) alarm clock, segment LCD
//     result.type = SwitchbotParseResult::TYPE_CGD1;
//     result.name = "CGD1";
//   } else if (device_uuid == 0x066F) {  // Cleargrass (Qingping) Temp & RH Lite
//     result.type = SwitchbotParseResult::TYPE_CGDK2;
//     result.name = "CGDK2";
//   } else if (device_uuid == 0x055b) {  // small square body, segment LCD, encrypted
//     result.type = SwitchbotParseResult::TYPE_LYWSD03MMC;
//     result.name = "LYWSD03MMC";
//   } else if (device_uuid == 0x1203) {  // small square body, e-ink display, encrypted
//     result.type = SwitchbotParseResult::TYPE_XMWSDJ04MMC;
//     result.name = "XMWSDJ04MMC";
//     if (raw.size() == 19)
//       result.raw_offset -= 6;
//   } else if (device_uuid == 0x07f6) {  // Switchbot-Yeelight BLE nightlight
//     result.type = SwitchbotParseResult::TYPE_MJYD02YLA;
//     result.name = "MJYD02YLA";
//     if (raw.size() == 19)
//       result.raw_offset -= 6;
//   } else if (device_uuid == 0x06d3) {  // rectangular body, e-ink display with alarm
//     result.type = SwitchbotParseResult::TYPE_MHOC303;
//     result.name = "MHOC303";
//   } else if (device_uuid == 0x0387) {  // square body, e-ink display
//     result.type = SwitchbotParseResult::TYPE_MHOC401;
//     result.name = "MHOC401";
//   } else if (device_uuid == 0x0A83) {  // Qingping-branded, motion & ambient light sensor
//     result.type = SwitchbotParseResult::TYPE_CGPR1;
//     result.name = "CGPR1";
//     if (raw.size() == 19)
//       result.raw_offset -= 6;
//   } else if (device_uuid == 0x0A8D) {  // Switchbot Mi Motion Sensor 2
//     result.type = SwitchbotParseResult::TYPE_RTCGQ02LM;
//     result.name = "RTCGQ02LM";
//     if (raw.size() == 19)
//       result.raw_offset -= 6;
//   } else {
//     ESP_LOGVV(TAG, "parse_switchbot_header(): unknown device, no magic bytes.");
//     return {};
//   }
...
// bool decrypt_switchbot_payload(std::vector<uint8_t> &raw, const uint8_t *bindkey, const uint64_t &address) {
//   if ((raw.size() != 19) && ((raw.size() < 22) || (raw.size() > 24))) {
//     ESP_LOGVV(TAG, "decrypt_switchbot_payload(): data packet has wrong size (%d)!", raw.size());
...
//   SwitchbotAESVector vector{.key = {0},
//                          .plaintext = {0},
//                          .ciphertext = {0},
//                          .authdata = {0x11},
//                          .iv = {0},
...
//   if (ret) {
//     ESP_LOGVV(TAG, "decrypt_switchbot_payload(): mbedtls_ccm_setkey() failed.");
...
//     ESP_LOGVV(TAG, "decrypt_switchbot_payload(): authenticated decryption failed.");
...
//   for (std::vector<uint8_t>::iterator it = raw.begin() + cipher_pos; it != raw.begin() + cipher_pos +
//   vector.datasize;
//        ++it) {
//     *it = *(p++);
//   }
...
//   ESP_LOGVV(TAG, "decrypt_switchbot_payload(): authenticated decryption passed.");
//   ESP_LOGVV(TAG, "  Plaintext : %s, Packet : %d", format_hex_pretty(raw.data() + cipher_pos,
//   vector.datasize).c_str(),
...
bool report_switchbot_results(const optional<SwitchbotParseResult> &result, const std::string &address) {
  if (!result.has_value()) {
    ESP_LOGVV(TAG, "report_switchbot_results(): no results available.");
    return false;
  }

  ESP_LOGD(TAG, "Got Switchbot %s (%s):", result->name.c_str(), address.c_str());
...
bool SwitchbotListener::parse_device(const esp32_ble_tracker::ESPBTDevice &device) {
  // Previously the message was parsed twice per packet, once by SwitchbotListener::parse_device()
  ...
  // Hence I disabled the call to parse_switchbot_header() here and the message parsing is done entirely
  // in the respective device instance. The SwitchbotListener class is defined in __init__.py and I was not
